...
## 2026-06 검수 라운드(280건) 분석에서 도출된 미결 — 사용자 확정 필요

### [x] ⭐ 세무 용어 통일: 이익세/급여세 vs 법인세/소득세 — **해결(2026-06-23)**
- 결정(사용자): **인사이트도 무조건 사이트 버전을 따른다** → 이익세→법인세, 급여세→소득세, 부기→기장 일괄.
- 반영: GLOSSARY 사이트 통일어로 갱신, RULES '용어 일관성: 사이트 버전이 마스터'. Phase 3에서 인사이트 본문 교체 적용.

### [ ] ⭐ 브랜드명 음차 '올리브 앤 바인' — 허용 범위
- 현황: 원칙은 `Olive & Vine` 영문 유지(GLOSSARY ✓). 그러나 검수에서 leadership·consulting·tax intro 본문에 음차 `올리브 앤 바인` 도입 → 3개 파일 혼용.
- 선택지: ① 음차 전면 철회(전부 Olive & Vine) ② 서사 본문은 음차 허용(첫 등장만 등) ③ 현행 혼용 유지
- 사유: 고유명사 일관성. **선호안?**

### [ ] 회사법 vs 회사조례(Companies Ordinance) 표기
- v5 확정 '홍콩 회사법' ↔ 검수본 '회사조례(Companies Ordinance)' 병기형(04·05). 인사이트는 '회사법'.
- 선택지: ① 회사법 통일 ② 회사조례(Companies Ordinance) 병기형 통일. **택1?**

### [ ] 통계 스트립 전면 영문 라벨 — 의도 확정
- 검수로 통계 라벨이 labelKo까지 전부 영문(Accounts Managed 등). 한국어 화면에서도 영문 노출 의도가 맞는지 확인. (현재는 검수=영문 확정으로 간주, RULES '영문 유지 면'에 반영함)
